...
const phoneDigits = (process.env.ADMIN_PHONE || "").replace(/\D/g, "");
const phone = phoneDigits.length === 11 ? `+55${phoneDigits}` : phoneDigits.startsWith("55") ? `+${phoneDigits}` : "";
...
if (!phone) throw new Error("Defina ADMIN_PHONE com um telefone brasileiro válido.");
...
const sql = `INSERT INTO users(id,email,phone,password_hash,password_algorithm,password_parameters,role,status,must_change_password,display_name,email_verified_at,created_at,updated_at) SELECT ${values} WHERE NOT EXISTS (SELECT 1 FROM users);\n`;
