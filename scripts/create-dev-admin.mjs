...
const salt=randomBytes(16),iterations=100000,hash=pbkdf2Sync(password+pepper,salt,iterations,32,"sha256").toString("base64"),parameters=JSON.stringify({iterations,salt:salt.toString("base64")}),now=new Date().toISOString(),quote=value=>`'${String(value).replaceAll("'","''")}'`;
