...
const disabledRegistrationPaths = new Set(["/cadastro", "/cadastro/", "/registro", "/registro/", "/register", "/register/"]);
...
export default { async fetch(request: Request, env: Env): Promise<Response> { const url=new URL(request.url),route=url.pathname.replace(/\/$/,"")||"/"; let response:Response; if(url.pathname.startsWith("/api/")) response=await api(request,env,url.pathname); else if(disabledRegistrationPaths.has(url.pathname)) response=Response.redirect(`${url.origin}/login`,302); else if(route==="/login"){ const auth=await currentUser(request,env); response=auth?Response.redirect(`${url.origin}${auth.user.must_change_password?"/perfil":"/inicio"}`,302):await env.ASSETS.fetch(request); } else if(route==="/inicio"||route==="/perfil"){ const auth=await currentUser(request,env); if(!auth) response=Response.redirect(`${url.origin}/login`,302); else if(route==="/inicio"&&auth.user.must_change_password) response=Response.redirect(`${url.origin}/perfil`,302); else response=await env.ASSETS.fetch(request); } else { response=await env.ASSETS.fetch(request); } return secureHeaders(assetCacheHeaders(response,url.pathname)); } };
